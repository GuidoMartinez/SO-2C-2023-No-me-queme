...
void *manejo_conexion_memoria_swap(void *arg)
{
    ...
    while (1)
    {
        ...
        case INICIO_SWAP:
            ...
            // MARCAR CANTIDAD DE BLOQUES COMO OCUPADOS
            // DEVOLVER POR A MEMORIA UNA LISTA DE INT con los bloques swap reservados con codigo de operacion LISTA_BLOQUES_SWAP;

            // para enviar la lista de INT (id_bloque_swap) podes usar la funcion serializar_lista_swap(t_list* , t _paquete*);
            break;
        case SWAP_A_LIBERAR:
            t_list *bloques_a_liberar = recibir_listado_id_bloques(socket_memoria);

            // ACA LIBERAS LOS BLOQUES -- LOS MARCAS COMO NO USADOS.

            // DEVOLVES un paquete con codigo de operacion SWAP_LIBERADA y un int (el que sea, es para que no quede bugeado el socket).
            break;
        ...
        case LEER_BLOQUE:
            ...
            // TRAER TODA LA INFO DEL BLOQUE (como void*) y lo mandas por socket a memoria. con el codigo VALOR_BLOQUE
